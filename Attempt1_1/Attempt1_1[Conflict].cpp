...
//std::uniform_real_distribution<double> normal(-100.0f, 3000.0f);
//std::normal_distribution<double> normal(1.0, 2000.0);
std::lognormal_distribution<double> normal(0.0, 7.0);
...
class Mesh
{
	private:
		...
		void diamond(int x, int y, int size, float* heightmap[], float displacement, int sides)
		{
			...
			if (firstRun)
			{
				heightmap[x + half_size][y + half_size] = max_height * 4;
				firstRun = false;
			}
			else heightmap[x + half_size][y + half_size] = heightBoundCheck(average + normal(generator)*displacement);
